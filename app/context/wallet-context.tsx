...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const handleAccountsChanged = useCallback(async () => {
    const accounts = await walletRef.current.wallet?.request({ 
  ...
  const handleNetworkChanged = useCallback(async () => {
    const chainId = await walletRef.current.wallet?.request({ 
  ...
  const disconnectStarknet = useCallback(async () => {
    try {
      ...
      if (walletRef.current.wallet?.off) {
        try {
          walletRef.current.wallet.off('accountsChanged', handleAccountsChanged)
        ...
        if (savedWallet) {
          try {
            ...
            if (data?.address && data?.isConnected) {
              ...
              const chainId = await connection.wallet.request({ 
                type: 'wallet_requestChainId' 
              }).catch(() => undefined)
  ...
  const currentAddress = useMemo(() => {
    ...
    return undefined
  }, [starknetWallet.isConnected, starknetWallet.address, privyAuthenticated, user?.wallet?.address])
