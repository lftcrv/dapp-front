...
export function DeployingState({ agent }: DeployingStateProps) {
  ...
  const checkContractStatus = useCallback(async () => {
    try {
      console.log(
        `[${new Date().toLocaleTimeString()}] Checking contract status for agent #${
          agent.id
        }...`,
      );

      // Use getCompleteAgentData for accurate contract info
      const result = await getCompleteAgentData(agent.id);

      if (result.success && result.data) {
        // Consider deployed if we have a valid contract address
        const isDeployed = Boolean(
          result.data.contractAddress &&
            result.data.contractAddress !== '0x0' &&
            result.data.contractAddress.length > 42, // Ensure it's a valid Starknet address
        );
        ...
        // If contract is deployed, redirect
        if (isDeployed) {
      ...
      // Reset countdown and update step
      setCountdown(20);
      setCurrentStep((prev) => (prev >= 5 ? 1 : prev + 1));
    } catch (error) {
      console.error('Error checking contract status:', error);
    }
  }, [agent.id, countdown, currentStep]);
  ...
  useEffect(() => {
    ...
    // Check every 20 seconds and store interval ID
    const intervals = [
      setInterval(checkContractStatus, 20000),
      // Update countdown every second
      setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 0) {
            // Trigger a new check when countdown reaches 0
            checkContractStatus();
            return 20;
          }
          return prev - 1;
    ...
    // Initial log
    console.log(
      `[${new Date().toLocaleTimeString()}] Started monitoring deployment:`,
      {
        agentId: agent.id,
    ...
    return () => {
      ...
      console.log(
        `[${new Date().toLocaleTimeString()}] Stopped monitoring deployment for agent #${
          agent.id
        }`,
      );
    };
  ...
  }, [agent.contractAddress, checkContractStatus]);
      ...
      <div className="container max-w-lg mx-auto px-4 text-center space-y-6">
        ...
        <div className="space-y-4">
          ...
          <p className="text-muted-foreground">
            The agent&apos;s smart contract is currently being deployed on the
            Starknet network. This process typically takes 1-2 minutes to
            complete.
          </p>
          <div className="space-y-4">
            {/* Steps list */}
            ...
            <div className="text-sm text-muted-foreground space-y-1">
              ...
              <p className="text-xs opacity-75">Next check in {countdown}s</p>
            </div>
          </div>
        </div>
      </div>
