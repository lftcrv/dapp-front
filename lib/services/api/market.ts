...
export async function getBondingMetrics(
  agentId: string,
): Promise<ApiResponse<BondingMetric>> {
  const response = await fetch(`${API_BASE}/bonding/${agentId}`);
  return response.json();
}

export async function getBondingHistory(
  agentId: string,
  ...
  const response = await fetch(
    `${API_BASE}/bonding/${agentId}/history?${queryParams.toString()}`,
  );
  return response.json();
}
...
export async function getMarketPrice(
  symbol: string,
): Promise<ApiResponse<OnChainPrice>> {
...
export async function getMarketPriceHistory(
  ...
  const response = await fetch(
    `${API_BASE}/price/history?${queryParams.toString()}`,
  );
  return response.json();
}
...
// Trending data
export async function getTrendingAgents(limit: number = 5): Promise<
  ApiResponse<
    Array<{
      agentId: string;
      symbol: string;
