...
const LoadingButton = memo(({ children, isLoading, ...props }: { children: React.ReactNode, isLoading: boolean } & React.ComponentProps<typeof Button>) => (
...
export const WalletConnectModal = memo(({ isOpen, onClose, onStarknetConnect }: WalletConnectModalProps) => {
  ...
  const handleStarknetConnect = async () => {
    ...
    try {
      ...
      if (wallet && connectorData?.account) {
        ...
        console.log('\x1b[36m%s\x1b[0m', `⏱️ Starknet Connection (Starknet): ${(performance.now() - startTime).toFixed(2)}ms`);
