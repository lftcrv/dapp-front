...
export const StarknetAccountDerivation = memo(function StarknetAccountDerivation() {
  ...
  const deriveAccount = useCallback(async () => {
    ...
    const evmWallet = wallets.find((w: ConnectedWallet) => w.address.toLowerCase() === privyAddress.toLowerCase());
    if (!evmWallet) {
    ...
    try {
      const account = await deriveStarknetAccount(privyAddress, async (message) => {
