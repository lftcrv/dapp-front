...
export function useProtocolFees() {
  ...
  const getUserSharePercentage = useCallback(() => {
    ...
    const leftCurveGainer = leftCurve.topGainers.find((g: Gainer) => g.address === address)
    const rightCurveGainer = rightCurve.topGainers.find((g: Gainer) => g.address === address)
