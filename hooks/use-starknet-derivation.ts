...
const DEBUG = {
  log: (message: string, ...args: unknown[]) => console.log('[Starknet Hook]:', message, ...args),
  error: (message: string, ...args: unknown[]) => console.error('[Starknet Hook Error]:', message, ...args)
}
    ...
    try {
      ...
      if (data?.users?.length > 0) {
        ...
        if (existingUser) {
          ...
          DEBUG.log('No existing derived user found, proceeding with derivation')
        }
      }

      // Find the connected EVM wallet
      const evmWallet = wallets.find((w: ConnectedWallet) => w.address.toLowerCase() === evmAddress.toLowerCase())
      if (!evmWallet) {
      ...
      const account = await deriveStarknetAccount(evmAddress, async (message: string) => {
